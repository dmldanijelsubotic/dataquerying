pipeline {
    ...
    stage('Prepare environment') {
        steps {
            bat """
                echo Preparing .env for Jenkins...
                if exist trunk\\jenkins.env.example (
                    copy /Y trunk\\jenkins.env.example trunk\\jenkins.env
                    echo Copied CI environment file.
                ) else (
                    echo WARNING: jenkins.env.example not found, using default or local .env
                )

                echo Preparing .env for Django...
                if exist trunk\\src\\dqm\\.env.example (
                    copy /Y trunk\\src\\dqm\\.env.example trunk\\src\\dqm\\.env
    ...
    stage('Load environment') {
        steps {
            script {
                def props = readProperties file: 'trunk/jenkins.env'
                props.each { k, v ->
                    env[k] = v
                }
            }
      ...
      steps {
        ...
        stage('Build') {
            steps {
                echo "Build"
                bat """
                "${env.SYS_PYTHON}" -m venv %VENV_DIR%
